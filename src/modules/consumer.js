...
const bmqConsumer = 'bmq:consumer';
  ...
  checkRpc(msg, queue) {
    ...
    return (content) => {
      if (msg.properties.replyTo) {
        ...
        this._connection.config.transport.info(bmqConsumer, `[${queue}][${msg.properties.replyTo}] >`, content);
    ...
    return this._connection.get()
      .then((channel) => {
        ...
        return this.channel.assertQueue(suffixedQueue, options)
          .then((q) => {
            this._connection.config.transport.info(bmqConsumer, 'init', q.queue);

            this.channel.consume(q.queue, (msg) => {
              this._connection.config.transport.info(bmqConsumer, `[${q.queue}] < ${msg.content.toString()}`);
              ...
              Promise.resolve(parsers.in(msg))
                ...
                .catch((err) => {
                  ...
                  this._connection.config.transport.error(bmqConsumer, err);
