...
var connect = function (url) {
  return new Promise(function (_resolve) {
    if (amqpState === amqpStates.DISCONNECTED) {
      ...
      amqpUrl = utils.getValidUrl([amqpUrl, url, amqpConfig.host, process.env.AMQP_URL, 'amqp://localhost']);
...
var consume = function(queue, options, callback) {
  ...
  .then(function (_q) {
    ...
    amqpChannel.consume(_q.queue, function (_msg) {
      ...
      if (!_msg.properties.headers['redelivered-ttl']) {
        _msg.properties.headers['redelivered-ttl'] = amqpConfig.redeliverTTL;
      }
...
var reject = function(queue, msg) {
  ++msg.properties.headers['redelivered-count'];

  var options = msg.properties;

  amqpChannel.reject(msg, false);
  if (msg.properties.headers['redelivered-ttl'] &&
      msg.properties.headers['redelivered-count'] <= msg.properties.headers['redelivered-ttl']) {
    amqpChannel.sendToQueue(queue, parsers.out(msg.content.toString(), options), options);
  }
};
